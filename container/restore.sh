...
if ! aws s3 cp "s3://${S3_BUCKET}/config/GameUserSettings.ini" "${ASA_INSTALL_DIR}/ShooterGame/Saved/Config/WindowsServer/GameUserSettings.ini"; then
...
if ! aws s3 cp "s3://${S3_BUCKET}/config/Game.ini" "${ASA_INSTALL_DIR}/ShooterGame/Saved/Config/WindowsServer/Game.ini"; then
